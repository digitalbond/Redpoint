...
---
-- Function to parse the response that was received from the S7 PLC
-- 
-- First argument is the socket that was created inside of the main Action
-- this will be utilized to send and receive the packets from the host.
-- the second argument is the query to be sent, this is passed in and is created
...
-- @param code The code that will be needed based on the packet, ONE or TWO is used for the order
-- @param output Table used for output for return to NMAP

function parse_response(response, host, port, code, output)
  ...
  if (value == 0x32) then
    ...
    if(code == "ONE") then
	  ...
	  -- set the version chars and separate them by "." if not a nil value
	  if (char1 ~= nil) then
	    version = char1 
		if(char2 ~= nil) then
		  version = version .. "." .. char2
		  if(char3 ~=nil) then
		    version = version .. "." .. char3
		  end
		end
	  else
		version = version .. ".0"	
	  end
      -- set version number in output table
	  output["Version"] = version
	  -- return the output table
      return output
	-- if the passed in CODE is TWO
